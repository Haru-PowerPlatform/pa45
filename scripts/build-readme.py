...
w("| 対象 | ライセンス | できること |")
w("|---|---|---|")
w("| コード（フローZIP・`scripts/`・`.github/`・サイトのCSS/JS） | [MIT License](LICENSE) | 改変・再配布・業務での利用 |")
w("| 教材（スライド・サイトの文章と図・アンケートの集計結果） | [CC BY-NC 4.0](LICENSE-CONTENT.md) | 出典を書けば、複製・改変・社内勉強会や研修での利用が可。販売は不可 |")
w("")
para(w, "いらすとやのイラスト、Microsoft 製品の画面やロゴ、参加者が書いたアンケートの自由記述は、どちらのライセンスの対象にも含めていません。詳しくは [LICENSE-CONTENT.md](LICENSE-CONTENT.md) にあります。")
w("")
...
f("- フローZIPは [MIT License](../LICENSE) です。改変・再配布・業務での利用ができます。")
f("")
...
s("- スライドは [CC BY-NC 4.0](../LICENSE-CONTENT.md) です。出典を書けば、社内勉強会や研修で使えます。販売は不可です。")
s("")
