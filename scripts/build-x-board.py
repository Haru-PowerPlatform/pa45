...
def load_extra_tab() -> dict:
    """リポジトリの外（Documents\\pa45-local\\board_extra_tab.py）に置いたローカル専用タブを読み込む。
    公開リポジトリに載せない運営メモ用。ファイルが無ければタブは出さない。"""
    f = ROOT.parent / "pa45-local" / "board_extra_tab.py"
    if not f.exists():
        return {}
    import importlib.util
    spec = importlib.util.spec_from_file_location("board_extra_tab", f)
...
<style>{CSS}{extra.get("css", "")}</style></head><body><div class="wrap">
<h1>X投稿ボード</h1>
