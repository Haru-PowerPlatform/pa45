...
def article2():
    ...
    c += P(f"そこで私が作ったのが、{BLUE('Teams のリアクションを自動でカウントして、週明け月曜にランキングを発表するフロー')}です。一番リアクションを集めた人へ「今週のナイスコメント賞です！おめでとうございます🎉」と自動でメッセージが届く。これだけで、投稿にもリアクションにも前向きな空気が生まれます。")
    ...
    return "【Power Automate】Teamsのリアクションを自動集計して「今週のナイスコメント賞」を発表する仕組み", c, [76, 78]
