...
**会話開始時に必ず確認**：
[`docs/USER_INSTRUCTIONS.md`](docs/USER_INSTRUCTIONS.md) — 過去にユーザー（はる）が出した指示・好み・ルールの集約。新セッションでも全項目を尊重する。

主な内容：
- 作業スタイル（許可確認なし・最後まで一気通貫）
- HTMLデザイン変更しない
- PA45フロー命名ルール
- ブログ記事の文体・MVP関連禁止
- X投稿のJSTとハッシュタグ
...
0. ⭐ **許可確認は絶対にしない**（最優先・全プロジェクト共通）。「進めていい？」「やる？」「OK？」と聞かない。ファイル編集・スクリプト実行・git commit/push・WordPress本番更新・connpass/X.com 操作 すべて 一気通貫で最後まで実行→最後に1回だけ報告。ユーザーから何度も明示済み「常に許可するから毎回聞かないで」「許可確認しないで」。Plan mode に入った場合のみ ExitPlanMode で1回だけ承認を取り、以降は連続実行OK。例外（必ず事前確認）：リポジトリ/DB削除など完全に元に戻せない操作、第三者へのメッセージ本番送信。
...
| 8 | 2026-04-30 | 391996 | 申請承認フロー（Approvals） | 41人 |
| 9 | 2026-05-07 | 392423 | SharePointファイル更新をTeamsに自動通知 | 35人 |
| 10 | 2026-05-14 | 393267 | 総復習回｜Forms × SPO × 承認 × Teamsを1本で（実務応用まとめ） | 28人 |
| 11 | 2026-05-21 | 393551 | 失敗しないフロー設計（実行履歴の読み方） | 開催前 |

### Vol.11 ハンズオン素材（2026-05-17 設計確定）
- **題材**: 「重要メール仕分けフロー」— メール受信トリガーの重要度で条件分岐
- **狙い**: 実行履歴の「出力」を読む価値を体感。「画面の表示」と「フローに渡る値」は違う、を学ぶ
- **フロー名**: PA45-No11-RunHistory ／ Solution UniqueName: PA45No11RunHistory ／ GitHub ZIP: `flows/vol-11/PA45-Vol11-RunHistory.zip`
- **フロー構成**:
  - トリガー: 新しいメールが届いたとき（Office 365 Outlook）
  - ① 条件: メールの「重要度」で分岐（まずわざと `"高"` と書く＝罠）
  - はい → 作成「⚠️ 重要メールです」／ いいえ → 作成「通常メール」
- **山場**: 重要度=高のメールを自分宛に送る → 条件 `"高"`≠`"High"` で false → 「通常メール」に行く → 「重要にしたのに！？」→ 実行履歴のトリガー出力を見ると `重要度（Importance）: "High"`（英語）→ 条件を `"High"` に直して解決
- **同じ構造の罠（スライド「今日のポイント」で紹介）**: 承認の結果＝`"Approve"`/`"Reject"`、SharePointのはい/いいえ列＝`true`/`false`、日時＝UTC
- **教える範囲**: 実行履歴の開き方／アクションの入力・出力の見方／「表示と値は違う」
- **教えない範囲**: 式・JSONの解析アクション・エラーハンドリング（情報過多になるため）

### Vol.10 ハンズオン素材
- **題材**: 経費申請の自動化（Forms→SP→Approvals→Teams）
- **Forms（経費申請フォーム）**: https://forms.cloud.microsoft/r/RpevgiKG2v
- **質問構成**: お名前／金額（円・数値制限）／用途／詳細（長答）
- **SharePointリスト**: 名前=`経費申請`／列=Title・Detail（複数行）・Status（選択肢：申請中／承認済／却下）
- **フロー名**: PA45-No10-BizReview
- **Solution UniqueName**: PA45No10BizReview
- **GitHub ZIP**: `flows/vol-10/PA45-Vol10-BizReview.zip`
...
| 投稿 | 状況 |
|------|------|
| 月曜 4/27 12:15「今週開催のお知らせ」 | ✅ 完了 |
| 木曜 4/30 12:15「当日案内」 | ✅ 完了 |
| 木曜 4/30 19:30「開催直前」 | ✅ 完了 |
...
### 既にある強み（2026-05時点・申請書類の核）
- **PA45 主催**：毎週木曜45分ハンズオン・10回連続開催・累計250名超・理解度90%超
- **広島PLUG運営**：地元コミュニティ運営
- **PPEC厳島（Nippon PPECの単発開催・2025-11 マツダ開催）**：そのイベントの **運営リーダー＋登壇者**。Nippon PPECコミュニティ全体の運営者ではなく、1回限りの「PPEC厳島」の運営を担当。エンタープライズ層公式コミュ。MS社員（藤原（ぎーくふじわら）氏）認知あり。**Microsoft 公式ブログにて登壇者として掲載予定**（=MVP申請の決定打になる外部証跡）
- **なんでもCopilot #80（2026-05-13）**：LT登壇「会議コストBot」
- **推薦できるMVP知人多数**：複数枠の推薦が見込める

### 残ボトルネック
1. 申請書類の「数字の見せ方」（achievements/に集約済 → 冒頭3行に数字を置く）
2. 認定資格（PL-300受験準備中、PL-200/400も将来）
3. 英語版コンテンツ1〜2本（DeepL+手直しで可）

### 申請ターゲット
- **2026-09四半期申請 → 12月発表組** が現実圏
- 9月までは PA45 を絶対に止めない（直前3ヶ月の活動が最も見られる）
