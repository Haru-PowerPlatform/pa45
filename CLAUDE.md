...
| `scripts/new-connpass-event.py --vol N --date Y --theme Z` | connpassイベント説明文を生成（貼り付け用テキスト出力） |
| `scripts/pre-event-checklist.py --vol N --date Y --event-id X --theme Z` | 開催前チェックリストを日数に応じて表示 |
...
- **MVP（Most Valuable Professional）審査・認定に関する文言は一切書かない**
  ...
  - はるさんはMVPを目指しているかもしれないが、ブログでアピールするのはしない方針
