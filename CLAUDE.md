...
| 10 | 2026-05-14 | 393267 | 総復習回｜Forms × SPO × 承認 × Teamsを1本で（実務応用まとめ） | 開催前 |
| 11 | 2026-05-21 | TBD | 実行履歴の出力を読めるとフローがつながる｜JSONとパスの見方 | 開催前 |

### Vol.11 ハンズオン素材
- **題材**: SharePoint「複数の項目の取得」→ 出力JSONを式で取り出す → Teamsへ集計通知
- **使うリスト**: Vol.10で作成した「経費申請」リストをそのまま流用
- **フロー名**: PA45-No11-RunHistory
- **Solution UniqueName**: PA45No11RunHistory
- **GitHub ZIP**: `flows/vol-11/PA45-Vol11-RunHistory.zip`
- **教える範囲**: 実行履歴の見方／出力JSONの読み方／`outputs() / first() / [0] / length()` の使い方
- **教えない範囲**: エラーハンドリング・並列分岐・「JSONの解析」アクション（情報過多になるため）
